...
app.use((req, res, next) => {
  ...
  if (req.path.startsWith('/api/admin/')) return requireAdmin(req, res, next)
  if (req.method === 'GET' && req.path === '/api/supp-checkouts') return requireAdmin(req, res, next)
  if (req.method === 'PUT' && req.path.startsWith('/api/supp-checkout/')) return requireAdmin(req, res, next)
  return next()
})
...
function addMemberToStore(store, rawName, rawPassword) {
  ...
  if (store.members.includes(name)) return { ok: false, msg: '该成员已存在' }
  const password = String(rawPassword || '').trim() || generateMemberPassword()
  if (password.length < 6) return { ok: false, msg: '初始密码至少6位' }
  store.members.push(name)
  store.passwords[name] = password
  store.memberEmails[name] = store.memberEmails[name] || ''
  store.memberProfiles[name] = store.memberProfiles[name] || {}
  return { ok: true, name, password }
}

function removeMemberFromNestedSchedule(store, name) {
  if (!store.schedule || typeof store.schedule !== 'object') return
  for (const day of Object.keys(store.schedule)) {
    const dayData = store.schedule[day]
    if (Array.isArray(dayData)) {
      store.schedule[day] = dayData.filter(n => n !== name)
      if (store.schedule[day].length === 0) delete store.schedule[day]
      continue
    }
    if (!dayData || typeof dayData !== 'object') continue
    for (const slotId of Object.keys(dayData)) {
      if (!Array.isArray(dayData[slotId])) continue
      dayData[slotId] = dayData[slotId].filter(n => n !== name)
      if (dayData[slotId].length === 0) delete dayData[slotId]
    }
  }
}

function removeMemberRelatedData(store, name) {
  removeMemberFromNestedSchedule(store, name)
  if (store.scheduleTime) {
    for (const key of Object.keys(store.scheduleTime)) {
      if (key.endsWith(`|${name}`)) delete store.scheduleTime[key]
    }
  }
  store.waitlist = (store.waitlist || []).filter(w => w.name !== name)
  store.cancelRequests = (store.cancelRequests || []).filter(r => r.name !== name)
  store.shiftSwapRequests = (store.shiftSwapRequests || []).filter(r => r.from !== name && r.to !== name)
  store.shiftSwapOverrides = (store.shiftSwapOverrides || []).filter(r => r.from !== name && r.to !== name)
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    if (password === ADMIN_PASSWORD) res.json({ ok: true, isAdmin: true, adminToken: createAdminToken(), name: '管理员' })
...
app.post('/api/admin/member', async (req, res) => {
  try {
    const { name, password } = req.body
    const store = await readStore()
    const added = addMemberToStore(store, name, password)
    if (!added.ok) return res.json(added)
    await writeStore({
      members: store.members,
    ...
    res.json({ ok: true, members: store.members, name: added.name, password: added.password })
  } catch (e) {
    res.status(500).json({ ok: false, msg: '服务器错误' })
  }
})
...
app.post('/api/admin/add-member', async (req, res) => {
  try {
    const store = await readStore()
    const added = addMemberToStore(store, req.body && req.body.name, req.body && req.body.password)
    if (!added.ok) return res.json(added)
    await writeStore({
      members: store.members,
      passwords: store.passwords,
...
app.post('/api/admin/member/remove', async (req, res) => {
  try {
    ...
    await writeStore({
      ...
      waitlist: store.waitlist,
      cancelRequests: store.cancelRequests,
      shiftSwapRequests: store.shiftSwapRequests,
      shiftSwapOverrides: store.shiftSwapOverrides,
      shiftSubstituteRequests: store.shiftSubstituteRequests,
...
app.post('/api/admin/assign', async (req, res) => {
  try {
    ...
    if (list.length >= (store.maxPerSlot || 1)) return res.json({ ok: false, msg: `该时段已满（${list.length}/${store.maxPerSlot || 1}）` })
    list.push(name)
...
app.post('/api/admin/waitlist/approve', async (req, res) => {
  try {
    ...
    if (!store.schedule[day][slotId]) store.schedule[day][slotId] = []
    const list = store.schedule[day][slotId]
    if (list.length >= (store.maxPerSlot || 1)) return res.json({ ok: false, msg: `该时段已满（${list.length}/${store.maxPerSlot || 1}），无法继续添加` })
...
app.get('/api/admin/passwords', async (req, res) => {
  try {
    const store = await readStore()
    const passwords = {}
    ;(store.members || []).forEach(name => { passwords[name] = '********' })
    res.json({ passwords, members: store.members || [] })
  } catch (e) {
    res.status(500).json({ ok: false, msg: '服务器错误' })
  }
})
...
function slotOverlapMinutes(inMin, outMin, slotStart, slotEnd, buf) {
  if (outMin === null || outMin === undefined) return 0
  const adjustedOut = outMin < inMin ? outMin + 24 * 60 : outMin
  const start = Math.max(inMin, slotStart - buf)
  const end = Math.min(adjustedOut, slotEnd + buf)
...
function checkinDistanceMeters(lat, lng) {
  const wgsDistance = haversineDistance(lat, lng, CHECKIN_LOCATION.lat, CHECKIN_LOCATION.lng)
  const gcjCenter = wgs84ToGcj02(CHECKIN_LOCATION.lat, CHECKIN_LOCATION.lng)
  const gcjDistance = haversineDistance(lat, lng, gcjCenter.lat, gcjCenter.lng)
  return Math.min(wgsDistance, gcjDistance)
}
...
app.post('/api/checkout/revoke', async (req, res) => {
  try {
    ...
    const diffMs = Date.now() - lastOutMs
      ...
      if (mp.lastOut) {
        ...
        if (scheduledSlots.length === 0) {
          ...
          for (const sid of scheduledSlots) {
            ...
            if (overlap > 0) {
              const h = Math.min(SLOT_HOURS[sid], Math.ceil((overlap / 60) * 2) / 2)
              rounded += h
              slotBreakdown.push({ slotId: sid, hours: h, overlapMinutes: overlap })
            }
          }
...
app.post('/api/supp-checkout', async (req, res) => {
  try {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date))) return res.json({ ok: false, msg: '日期格式不正确' })
    if (!/^\d{1,2}:\d{2}$/.test(String(time))) return res.json({ ok: false, msg: '时间格式不正确' })
    const [hh, mm] = String(time).split(':').map(Number)
    if (hh < 0 || hh > 23 || mm < 0 || mm > 59) return res.json({ ok: false, msg: '时间范围不正确' })
    const store = await readStore()
    if (!store.members.includes(name)) return res.json({ ok: false, msg: '成员不存在' })
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      store.checkins.push({
        ...
        name: item.name,
        date: item.date,
      time: String(item.time).length === 5 ? item.time + ':00' : item.time,
        type: 'out',
        isSupp: true,
        latitude: null,
        longitude: null,
